...
class backup_restore{
    ...
    // Constructor
 	function __construct($dbhost,$database,$dbUser ,$dbPass ,$tables="*" ) {
		...
		$this->host = $dbhost;
 		...
 		$this->user = $dbUser;
 		$this->pass = $dbPass ;
 		$this->file = "Backups\Backups\backup ".date("Y-m-d H.i.s").".sql";
 		$this->tables =$tables;
	    $this->msg='';
	 	}
  ...
  //Ohh Wait ,When we create backup. lets save this time in log file
	if (file_exists('Backup\\backup_log.log'))
	{
		$hnd =fopen('Backup\\backup_log.log','a');
  ...
  //yeah its all done
    ...
    //Restore
 	public function restore($path) {
			...
			$this->Connect();
 			$this->file = "Backups\\".$path;
			if (file_exists($this->file))
 			$lines = file($this->file, FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
			else {
			return "File is missing ,Please sellect exsisting backup file";
			exit;
			}
